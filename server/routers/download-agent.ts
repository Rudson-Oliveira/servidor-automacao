...
/**
 * Router para download de arquivos do Desktop Agent
 */
export const downloadAgentRouter = router({
  /**
   * Baixar agent.py
   */
  getAgentPy: publicProcedure.query(() => {
    const TOKEN = "86fa95160005ff2e3e971acf9d8620abaa4a27bc064e7b8a41980dbde6ea990e";
    const SERVER_URL = process.env.VITE_APP_URL || "https://3000-irvlht34m10g6oxfkoitw-1b347671.manusvm.computer";
    const WS_URL = SERVER_URL.replace("https://", "wss://").replace("http://", "ws://") + "/ws/desktop-agent";

    const agentPyContent = `#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
  ...
  getInstallerBat: publicProcedure.query(() => {
    ...
    if (!fs.existsSync(batPath)) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: "Arquivo INSTALADOR_COMPLETO.bat não encontrado",
      });
    }
